...
// ----------------------------------------------------------------------------------
//$dir = get_stylesheet_directry_uri();
...
function create_post_type() {
    register_post_type( 'jiseki', [ // 投稿タイプ名の定義
        ...
        'show_in_rest'  => false,  // 5系から出てきた新エディタ「Gutenberg」を有効にする
				'supports' => array('title','thumbnail') //アイキャッチ、本文非表示
    ]);
		register_taxonomy_for_object_type('post_tag','jiseki'); //タグも表示されるようにする
}

function melos_thinkup_child_frontscripts() {

	wp_enqueue_style( 'glid', get_stylesheet_directory_uri() . '/glid.css');
	wp_enqueue_style( 'style', get_template_directory_uri() . '/style.css', array( 'thinkup-bootstrap' ));
}
